...
export function createSvgObjectsFromPcbKeepout(
  ...
  for (const layer of keepout.layers) {
    ...
    if (keepout.shape === "rect") {
      ...
      const safeRadius =
        typeof circleKeepout.radius === "number" && Number.isFinite(circleKeepout.radius)
          ? Math.max(0, circleKeepout.radius)
          : 0
      const scaledRadius = safeRadius * Math.abs(transform.a)
