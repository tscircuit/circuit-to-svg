...
export function createSvgObjectsFromPcbVia(
  ...
  const rawOuter =
    typeof hole.outer_diameter === "number" && Number.isFinite(hole.outer_diameter)
      ? Math.max(0, hole.outer_diameter)
      : 0
  const rawHole =
    typeof hole.hole_diameter === "number" && Number.isFinite(hole.hole_diameter)
      ? Math.max(0, hole.hole_diameter)
      : 0
  const scaledOuterWidth = rawOuter * Math.abs(transform.a)
  const scaledOuterHeight = rawOuter * Math.abs(transform.a)
  const scaledHoleWidth = rawHole * Math.abs(transform.a)
  const scaledHoleHeight = rawHole * Math.abs(transform.a)
