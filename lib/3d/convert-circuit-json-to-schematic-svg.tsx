...
export function convertCircuitJsonTo3dSvg(
  ...
  camera ??= soup.some(e => e.layer === "bottom") ? ["top", "bottom"] : ["top"]
  ...
  const root = createRoot({})
    .configure({
      gl: new SVGRenderer, frameloop: "never",
      size: { width, height, top: 0, left: 0 },
    })

  // ensure DOM updated immediately
  const state = flushSync(() => root.render(<>
  ...
  for (let i = 0; i < svgs.length; i++) {
    switch (camera[i]) {
      case "top": state.camera.position.z = zoom; break
      case "bottom":
