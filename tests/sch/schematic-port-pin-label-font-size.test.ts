...
test("styled pin-label parts overline only their own substring", () => {
  ...
  expect(label.children[0]?.attributes["text-decoration"]).toBeUndefined()
  expect(label.children[1]?.attributes["text-decoration"]).toBe("overline")
  expect(label.children[2]?.attributes["text-decoration"]).toBeUndefined()
})
