...
test("silkscreen text uses alphabet path geometry", () => {
  ...
  const strokeWidth = svg.match(
    /stroke-width="([^"]+)"[^>]+class="pcb-silkscreen-text/,
  )?.[1]

  expect(svg).toContain('<path d="M')
  expect(svg).not.toContain("font-family")
  expect(Number(strokeWidth)).toBeCloseTo(0.0432, 4)
})
